...
#     auxiliary_output = Dense(1, activation='sigmoid', name='aux_output')(lstm_out1)
#
...
def prediction(model, test_X, test_X2, test_y, timestamps, n_columns, scaler):
...
def plot(y_true, y_predict,Smape):
    ...
    plt.title('Actual and Predicted ' + 'SO2_ ' + 'Smape:'+ Smape )
    plt.savefig('Results/MultiTask_SingleLabel_Model_predicted_and_actural_'+'SO2'+'.eps', format="eps", dpi=300)
    plt.show()


def main():
    train_data = 'data/preprocessed_data/bj_daxing.csv'
    train_data2 = 'data/preprocessed_data/bj_fangshan.csv'
    # train_data3 = 'data/preprocessed_data/bj_mentougou.csv'
    ...
    test_data2 = 'data/preprocessed_data/test/bj_fangshan_201805.csv'
    # test_data3 = 'data/preprocessed_data/test/bj_mentougou_201805.csv'
    ...
    train_X2, train_y2 = split_dataset(scaled_train2, look_back, n_columns)
    # train_X3, train_y3 = split_dataset(scaled_train3, look_back, n_columns)
    # train_X4, train_y4 = split_dataset(scaled_train4, look_back, n_columns)
    # train_X5, train_y5 = split_dataset(scaled_train5, look_back, n_columns)
    # train_X6, train_y6 = split_dataset(scaled_train6, look_back, n_columns)
    # train_X7, train_y7 = split_dataset(scaled_train7, look_back, n_columns)
    # train_X8, train_y8 = split_dataset(scaled_train8, look_back, n_columns)
    # train_X9, train_y9 = split_dataset(scaled_train9, look_back, n_columns)

    test_X, test_y = split_dataset(scaled_test, look_back, n_columns)
    test_X2, test_y2 = split_dataset(scaled_test2, look_back, n_columns)
    # test_X3, test_y3 = split_dataset(scaled_test3, look_back, n_columns)
    # test_X4, test_y4 = split_dataset(scaled_test4, look_back, n_columns)
    # test_X5, test_y5 = split_dataset(scaled_test5, look_back, n_columns)
    # test_X6, test_y6 = split_dataset(scaled_test6, look_back, n_columns)
    # test_X7, test_y7 = split_dataset(scaled_test7, look_back, n_columns)
    # test_X8, test_y8 = split_dataset(scaled_test8, look_back, n_columns)
    # test_X9, test_y9 = split_dataset(scaled_test9, look_back, n_columns)
    ...
    # make a prediction
    y_predict, y_true = prediction(model, test_X, test_X2, test_y, look_back, n_columns, scaler_test)
