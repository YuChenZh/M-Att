...
from numpy import concatenate as npconca
...
from keras.layers import Input, Embedding, Dense,Convolution1D,MaxPooling1D,merge,dot,concatenate
...
def attention_3d_block(inputs,shared):
    ...
    # sent_representation = Lambda(lambda xin: K.sum(xin, axis=-2), output_shape=(input_dim,))(sent_representation)

    # #attention 2
    input_dim = int(inputs.shape[2])
    activations_weights = Flatten()(shared)
    activations_weights = Dense(TIME_STEPS, activation='tanh')(activations_weights)
    activations_weights = Activation('softmax')(activations_weights)
    activations_weights = RepeatVector(input_dim)(activations_weights)
    activations_weights = Permute([2, 1])(activations_weights)
    activations_weighted = keras.layers.multiply([inputs, activations_weights])

    # sent_representation = Lambda(lambda x: K.sum(x, axis=-2), output_shape=(input_dim,))(activations_weighted)
...
def build_model(trainX_list,
    ...
    print('=============================================================')
    similarity1,similarity2 = tensor_similarity(input_list[0], input_list[1], trainX_list[5],trainX_list[1])
    print(np.average(similarity1))
    print(np.average(similarity2))
    ...
    output_list = []
    for i in range(0,task_num):
        locals()['concate_layer'+str(i)] =  attention_3d_block(locals()['cnn_out'+str(i)], concate_layer)
        locals()['flatten_layer'+str(i)] = LSTM(dense_num,activation='relu')(locals()['concate_layer'+str(i)])
        # locals()['sub'+str(i)] = Dense(dense_num,activation='relu')(locals()['concate_layer'+str(i)])
        locals()['out'+str(i)] = Dense(n_labels, activation='sigmoid')(locals()['flatten_layer'+str(i)])
...
def evaluation(test_X, test_y, y_pred, timestamps, n_columns, n_labels, scaler):
    ...
    y_predict = npconca((test_X[:, -n_columns:-n_labels], y_pred), axis=1)
    y_predict = scaler.inverse_transform(y_predict)
    y_predict = y_predict[:, -n_labels:]
    # invert scaling for actual
    test_y = test_y.reshape((len(test_y), n_labels))
    y_true = npconca((test_X[:, -n_columns:-n_labels], test_y), axis=1)
...
def main():
    ...
    file = open('results/Attention_results(6)3.txt', 'w')
